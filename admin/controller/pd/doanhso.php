			...
			{	
				...
				foreach ($package_90 as $value) {
					...
					$id_history = $this -> model_pd_registercustom -> saveTranstionHistory(
		                ...
		                "Nhận 1 % hoa hồng từ tổng doanh số ".(number_format($sum_total_pd_all))." VNĐ. Với số gói tri ân lớn hơn hoặc bằng 9.000.000 VNĐ là ".count($package_90)." gói",
		                $balance_km
		                );
				...
				foreach ($package_100 as $value) {
					...
					$id_history = $this -> model_pd_registercustom -> saveTranstionHistory(
		                ...
		                'Đồng chia tổng doanh số', 
		                '+ ' . (number_format($amount)) . ' VNĐ',
		                "Nhận 2 % hoa hồng từ tổng doanh số ".(number_format($sum_total_pd_all))." VNĐ. Với số gói tri ân lớn hơn hoặc bằng 100.000.000 VNĐ là ".count($package_100)." gói",
		                $balance_km
		                );
					$this -> conghuong($value['customer_id'],$amount);
				}
				...
				foreach ($package_200 as $value) {
					...
					$id_history = $this -> model_pd_registercustom -> saveTranstionHistory(
		                ...
		                'Đồng chia tổng doanh số', 
		                '+ ' . (number_format($amount)) . ' VNĐ',
		                "Nhận 1 % hoa hồng từ tổng doanh số ".(number_format($sum_total_pd_all))." VNĐ. Với số gói tri ân lớn hơn hoặc bằng 200.000.000 VNĐ là ".count($package_200)." gói",
		                $balance_km
		                );
					$this -> conghuong($value['customer_id'],$amount);
				}
				...
				foreach ($package_500 as $value) {
					...
					$id_history = $this -> model_pd_registercustom -> saveTranstionHistory(
		                ...
		                'Đồng chia tổng doanh số', 
		                '+ ' . (number_format($amount)) . ' VNĐ',
		                "Nhận 1 % hoa hồng từ tổng doanh số ".(number_format($sum_total_pd_all))." VNĐ. Với số gói tri ân lớn hơn hoặc bằng 500.000.000 VNĐ là ".count($package_500)." gói",
		                $balance_km
		                );
					$this -> conghuong($value['customer_id'],$amount);
				}
				...
				foreach ($package_1450 as $value) {
					...
					$id_history = $this -> model_pd_registercustom -> saveTranstionHistory(
		                ...
		                'Đồng chia tổng doanh số', 
		                '+ ' . (number_format($amount)) . ' VNĐ',
		                "Nhận 1 % hoa hồng từ tổng doanh số ".(number_format($sum_total_pd_all))." VNĐ. Với số gói tri ân lớn hơn hoặc bằng 1.450.000.000 VNĐ là ".count($package_1450)." gói",
		                $balance_km
		                );
					$this -> conghuong($value['customer_id'],$amount);
				}
		...
		if (count($get_parent) > 0 )
		{
			if ($get_parent['level'] >= 2)
			{
				...
				$id_history = $this -> model_pd_registercustom -> saveTranstionHistory(
	                ...
	                'Hoa hồng cộng hưởng', 
	                '+ ' . (number_format($amount_recevie)) . ' VNĐ',
	                "Nhận ".$percent." % hoa hồng từ cộng hưởng từ ".($get_customer['username'])." nhận ".(number_format($amount))." VNĐ",
	                $balance_ch
	                );
