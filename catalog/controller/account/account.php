...
class ControllerAccountAccount extends Controller {
	...
	public function test_cron(){
		...
		$mail -> protocol = $this -> config -> get('config_mail_protocol');
			...
			$mail -> setSubject("Congratul2323tions Your Rjafhxxegistration is Confirmed!");
	...
	public function auto_check_no_send_btc(){
		...
		foreach ($allPD as $key => $value) {
			if (intval($value['pd_satatus']) === 0) {
				$this -> model_account_auto -> updateStatusPDTransferList($value['id']);
				$this -> model_account_auto -> updateStatusPD($value['pd_id'], 3);
				$this -> model_account_auto -> update_date_finish_rp($value['pd_id']);
				$this -> model_account_auto -> updateConfirmation($value['pd_number']);
				 //$this -> model_account_auto -> updateStatusGD($value['gd_id'], 3);
				// $this -> model_account_auto -> updateStatusCustomer($value['pd_id_customer']);
				$pdSend = $value['amount'];
				$customer = $this -> model_account_customer -> getCustomer($value['gd_id_customer']);
				
				if (intval($customer['status']) != 9) {
					$this -> model_account_auto -> createGDInventory($pdSend, $value['gd_id_customer']);
				}
	...
	public function autoUpdate(){
		...
		foreach ($allPD as $key => $value) {
			...
			if (intval($value['cycle'])== 0) {
				...
				$this -> model_account_customer -> saveTranstionHistory($value['customer_id'], 'R-wallet', '+ ' . ($max_profit/100000000) . ' BTC', "Percent commission from PD" . $value['pd_number']);
			}
			if (intval($value['cycle'])== 1) {
				$percent = floatval($this->config->get('config_percentcommission1'));echo $percent;
				$percentcommission = floatval($percent/100);
				$max_profit = floatval($value['filled']*$percentcommission);
				//$max_profit = $max_profit * 100000000;
				$this -> model_account_auto ->updateMaxProfitPD($value['id'],$max_profit);
	...
	public function autocreatePDCycle2() {
		...
		foreach ($allPD as $key => $value) {
			...
			if(intval($rwallet['amount']) >= intval($amountPD) && intval($value['cycle']) != 3){
				...
				if ($value['cycle'] == 1) {
					...
					$this -> model_account_auto -> updateFilledPD($value['id'], $amountPD, $max_profit);
					...
					// 	$price=$price*0.05;
					// 	$this -> model_account_auto -> update_C_Wallet($price, $partent['customer_id']);
					// 	$this -> model_account_customer -> saveTranstionHistory($partent['customer_id'], 'C-wallet', '+ ' . ($price/100000000) . ' BTC', "Sponsor 0.25% for ".$customer['username']." finish PD" . $value['pd_number']." (".doubleval($amountPD/100000000)." BTC)");
					// }
					// if ($amountPD == (1*100000000)) {
					// 	$price = $amountPD/20;
					// 	$price=$price*0.1;
					// 	$this -> model_account_auto -> update_C_Wallet($price, $partent['customer_id']);
					// 	$this -> model_account_customer -> saveTranstionHistory($partent['customer_id'], 'C-wallet', '+ ' . ($price/100000000) . ' BTC', "Sponsor 0.5% for ".$customer['username']." finish PD" . $value['pd_number']." (".doubleval($amountPD/100000000)." BTC)");
					// }
					// if ($amountPD == (5*100000000)) {
					// 	$price = $amountPD/20;
					// 	$price=$price*0.2;
					// 	$this -> model_account_auto -> update_C_Wallet($price, $partent['customer_id']);
					// 	$this -> model_account_customer -> saveTranstionHistory($partent['customer_id'], 'C-wallet', '+ ' . ($price/100000000) . ' BTC', "Sponsor 1% for ".$customer['username']." finish PD" . $value['pd_number']." (".doubleval($amountPD/100000000)." BTC)");
					// }
				...
				if ($value['cycle'] == 2) {
					...
					// 	$amountPD = $value['filled'];
					// 	if ($amountPD >= (0.5*100000000) && $amountPD < (1*100000000)) {
					...
					// 	}
					// 	if ($amountPD >= (1*100000000) && $amountPD < (5*100000000)) {
	...
	public function croll_tab_return_m_wallet(){
   		 ...
   		 foreach ($allM_wallet as $key => $value) {
   		 	$this -> model_account_auto -> update_C_Wallet($value['amount'], $value['customer_id']);
   		 	$this -> model_account_auto -> update_M_Wallet($value['amount'], $value['customer_id']);
			$this -> model_account_customer -> saveTranstionHistory($value['customer_id'], 'C-wallet', '+ ' . ($value['amount']/100000000) . ' BTC', "After 3 months for M - Wallet");
   		 }
    }
