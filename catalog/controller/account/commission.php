...
class ControllerAccountCommission extends Controller {
	...
	public function loinhuan() {
		...
		$data['pds'] = $this -> model_account_customer -> getallcommision_loinhuan($this -> customer -> getId(), $limit, $start);
	...
	public function dt() {
		...
		$limit = 10;
		$start = ($page - 1) * 10;
		$pd_total = $this -> model_account_customer -> getTotalcommission_dt($this -> customer -> getId());
		...
		$pagination -> num_links = 5;
		$pagination -> text = 'text';
		$pagination -> url = 'dt.html&page={page}';

		$data['pds'] = $this -> model_account_customer -> getallcommision_dt($this -> customer -> getId(), $limit, $start);
		
		$data['pagination'] = $pagination -> render();

		if (file_exists(DIR_TEMPLATE . $this -> config -> get('config_template') . '/template/account/commission_dt.tpl')) {
			$this -> response -> setOutput($this -> load -> view($this -> config -> get('config_template') . '/template/account/commission_dt.tpl', $data));
		} else {
			$this -> response -> setOutput($this -> load -> view('default/template/account/commission_dt.tpl', $data));
		}
	}
