<?php
class ModelAccountGd extends Model {
	...
	public function getAllInvoiceByCustomer($customer_id, $limit, $offset){
		$query = $this -> db -> query("
			...
			FROM ". DB_PREFIX ."customer_invoice_fee
	...
	public function updateInaddressAndFree($invoice_id, $invoice_id_hash , $input_addr, $fee_percent, $my_addr, $call_back){
		$query = $this -> db -> query("
			UPDATE " . DB_PREFIX . "customer_invoice_fee SET
...
public function getPD_By_GD_Numner($pd_number){
		$query = $this -> db -> query("
			SELECT status
			FROM ". DB_PREFIX ."customer_get_donation
			WHERE gd_number = '".$pd_number."'
			");
		return $query -> row;
	}
	...
	public function saveInvoice($customer_id, $secret, $amount, $pd_id){
		$query = $this -> db -> query("
			INSERT INTO ".DB_PREFIX."customer_invoice_fee SET
	...
	public function getGDBefore(){
		$query = $this -> db -> query("
			...
			FROM ". DB_PREFIX . "customer_get_donation 
			WHERE status IN (0,1) AND filled < amount AND customer_id <> ".$this -> customer -> getId()."
	...
	public function getCustomerInventory(){
		$query = $this -> db -> query("
			...
			WHERE status = 9 AND customer_id <> '".$this -> customer -> getId()."'
			ORDER BY date_added ASC 
			LIMIT 1
		");
		return $query -> row;
